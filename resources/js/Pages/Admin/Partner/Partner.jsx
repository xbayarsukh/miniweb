...
export default function Partner({ partners }) {
    ...
    const [selectedPartners, setSelectedPartners] = useState(null); // Store the selected partners
    const { post } = useForm({});

    const openModal = (partners) => {
        setSelectedPartners(partners); // Set the selected partners for modal
        setIsOpen(true);
    };

    const closeModal = () => {
        setSelectedPartners(null); // Reset selected partners when modal is closed
        setIsOpen(false);
    };
    ...
    const submitDelete = async () => {
        try {
            let response = await post(
                route("admin.partners.destroy", selectedPartners.id)
            );
            closeModal();
        } catch (error) {
    ...
    return (
        <AuthenticatedLayout header="Хамтрагч байгууллага">
                                        ...
                                        <tbody>
                                            {partners.map((partner) => (
                                                <tr key={partner.id}>
                                                    ...
                                                    <td className="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        <div className="text-xs font-semibold leading-tight text-slate-400">
                                                            {partner.link}
                                                        </div>
                                                    </td>
                                                    <td className="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        <span className="text-xs font-semibold leading-tight text-slate-400">
                                                            {(() => {
                                                                const timestamp = partner.created_at;  // UTC timestamp
                                                    ...
                                                    <td className="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        <span className="text-xs font-semibold leading-tight text-slate-400">
                                                            {(() => {
                                                                const timestamp = partner.updated_at;  // UTC timestamp
                                                                // Parse the UTC timestamp and convert it to the desired time zone
                                                                const date = DateTime.fromISO(timestamp, { zone: 'utc' }).setZone('Asia/Ulaanbaatar');
                                                    ...
                                                    <td className="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        ...
                                                        <a
                                                            onClick={() =>
                                                                openModal(partner)
                                                            } // Open modal with selected partners
                    ...
                    >
                        <div className="flex justify-between items-start">
                            <h2 className="text-xl font-semibold">
                                <b>{selectedPartners.title}</b> Та энэ мэдээг
