...
export default function CreateAndEdit({ singlePartner }) {
    const isEditMode = !!singlePartner;
    const [image, setImage] = useState(singlePartner == null ? null: ("/" + singlePartner.image));
    const { data, setData, post, errors, processing, recentlySuccessful } =
        useForm({
            title: singlePartner?.title || '',
            link: singlePartner?.link || '',
            image: null,
            content: singlePartner?.content || '',
        });
    ...
    const submit = async (e) => {
        ...
        try {
            let response;
            if (isEditMode) {
                response = await post(route("admin.partners.update", singlePartner.id));
            } else {
                response = await post(route("admin.partners.store"));
            }
                            ...
                            <ul className="flex flex-col pl-0 mb-0 rounded-lg">
                                ...
                                <li className="relative block px-0 py-2 bg-white border-0 rounded-t-lg text-inherit">
                                    <div className="min-h-6 mb-0.5 pl-0 flex flex-row items-center">
                                        ...
                                        <input
                                            type="text"
                                            value={data.link} // Binds to data.title from your state
                                            onChange={(e) =>
                                                setData("link", e.target.value)
                                            }
                                            className="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-6/12 appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                                            placeholder="Линк" // Update placeholder text as needed
                                            aria-label="Link" // Updated the aria-label for accessibility
                                            aria-describedby="title-addon" // Updated aria-describedby for proper labeling
                                        />
                                    </div>
                                </li>
