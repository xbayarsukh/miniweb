...
export default function Service({ services }) {
    ...
    const openModal = (service) => {
        setSelectedService(service); // Set the selected service for modal
        setIsOpen(true);
    };

    const closeModal = () => {
        setSelectedService(null); // Reset selected service when modal is closed
        setIsOpen(false);
    };
    ...
    const submitDelete = async () => {
        try {
            let response = await service(
                route("admin.services.destroy", selectedService.id)
            );
            closeModal();
        } catch (error) {
    ...
    return (
        <AuthenticatedLayout header="Үйлчилгээ">
                                                ...
                                                <tr key={service.id}>
                                                    ...
                                                    <td className="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        <span className="text-xs font-semibold leading-tight text-slate-400">
                                                            {(() => {
                                                                const timestamp = service.created_at;  // UTC timestamp
                                                    ...
                                                    <td className="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        <span className="text-xs font-semibold leading-tight text-slate-400">
                                                            {(() => {
                                                                const timestamp = service.updated_at;  // UTC timestamp
                                                                // Parse the UTC timestamp and convert it to the desired time zone
                                                                const date = DateTime.fromISO(timestamp, { zone: 'utc' }).setZone('Asia/Ulaanbaatar');
                    ...
                    >
                        <div className="flex justify-between items-start">
                            <h2 className="text-xl font-semibold">
                                <b>{selectedService.title}</b> Та энэ Үйлчилгээг
