...
export default function Portfolio({ portfolios }) {
    ...
    const [selectedPortfolios, setSelectedPortfolios] = useState(null); // Store the selected portfolios
    const { post } = useForm({});

    const openModal = (portfolios) => {
        setSelectedPortfolios(portfolios); // Set the selected portfolios for modal
        setIsOpen(true);
    };

    const closeModal = () => {
        setSelectedPortfolios(null); // Reset selected portfolios when modal is closed
        setIsOpen(false);
    };
    ...
    const submitDelete = async () => {
        try {
            let response = await post(
                route("admin.portfolios.destroy", selectedPortfolios.id)
            );
            closeModal();
        } catch (error) {
    ...
    return (
        <AuthenticatedLayout header="Portfolio">
                                            ...
                                            __html: portfolio.content,
                                                    ...
                                                    <td className="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        <span className="text-xs font-semibold leading-tight text-slate-400">
                                                            {(() => {
                                                                const timestamp = portfolio.created_at;  // UTC timestamp
                                                    ...
                                                    <td className="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        <span className="text-xs font-semibold leading-tight text-slate-400">
                                                            {(() => {
                                                                const timestamp = portfolio.updated_at;  // UTC timestamp
                                                                // Parse the UTC timestamp and convert it to the desired time zone
                                                                const date = DateTime.fromISO(timestamp, { zone: 'utc' }).setZone('Asia/Ulaanbaatar');
                                                    ...
                                                    <td className="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        ...
                                                        <a
                                                            onClick={() =>
                                                                openModal(portfolio)
                                                            } // Open modal with selected portfolios
                    ...
                    >
                        <div className="flex justify-between items-start">
                            <h2 className="text-xl font-semibold">
                                <b>{selectedPortfolios.title}</b> Та энэ мэдээг
