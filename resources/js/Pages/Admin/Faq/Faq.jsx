...
export default function Faq({ faqs }) {
    ...
    const [selectedFaqs, setSelectedFaqs] = useState(null); // Store the selected faqs
    const { post } = useForm({});

    const openModal = (faqs) => {
        setSelectedFaqs(faqs); // Set the selected faqs for modal
        setIsOpen(true);
    };

    const closeModal = () => {
        setSelectedFaqs(null); // Reset selected faqs when modal is closed
        setIsOpen(false);
    };
    ...
    const submitDelete = async () => {
        try {
            let response = await post(
                route("admin.faqs.destroy", selectedFaqs.id)
            );
            closeModal();
        } catch (error) {
    ...
    return (
        <AuthenticatedLayout header="Түгээмэл асуулт хариулт">
                                        ...
                                        <tbody>
                                            {faqs.map((faq) => (
                                                <tr key={faq.id}>
                                                    ...
                                                    <td className="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent !w-[200px]">
                                                        <div style={{overflowWrap: 'initial', width: '40%'}} dangerouslySetInnerHTML={{
                                                            __html: faq.content,
                                                        }}></div>
                                                    </td>
                                                    <td className="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        <span className="text-xs font-semibold leading-tight text-slate-400">
                                                            {(() => {
                                                                const timestamp = faq.created_at;  // UTC timestamp
                                                    ...
                                                    <td className="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                        <span className="text-xs font-semibold leading-tight text-slate-400">
                                                            {(() => {
                                                                const timestamp = faq.updated_at;  // UTC timestamp
                                                                // Parse the UTC timestamp and convert it to the desired time zone
                                                                const date = DateTime.fromISO(timestamp, { zone: 'utc' }).setZone('Asia/Ulaanbaatar');
                    ...
                    >
                        <div className="flex justify-between items-start">
                            <h2 className="text-xl font-semibold">
                                <b>{selectedFaqs.title}</b> Та энэ мэдээг
