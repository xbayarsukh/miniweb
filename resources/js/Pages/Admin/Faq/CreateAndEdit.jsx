...
export default function CreateAndEdit({ singleFaq }) {
    const isEditMode = !!singleFaq;
    const [image, setImage] = useState(singleFaq == null ? null: ("/" + singleFaq.image));
    ...
    const submit = async (e) => {
        ...
        try {
            let response;
            if (isEditMode) {
                response = await post(route("admin.faqs.update", singleFaq.id));
            } else {
                response = await post(route("admin.faqs.store"));
            }
            ...
            <div className="w-10/12 max-w-10/12 px-3 mx-auto mt-10">
                <div className="relative flex flex-col h-full min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
                    <div className="p-4 pb-0 mb-0 bg-white border-b-0 rounded-t-2xl">
                        <h6 className="mb-0">{isEditMode ? 'Түгээмэл асуулт хариулт засах' : 'Түгээмэл асуулт хариулт нэмэх'}</h6>
                    </div>
                    <form
                        onSubmit={submit}
